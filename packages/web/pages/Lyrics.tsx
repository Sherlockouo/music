    ...
    `)
        ...
        return (
            <div className={`${lineClassName} ${lyricRowCss}`} key={index} onDoubleClick={setSongToLyric}>
                <p className={'pointer-events-none'}>{lyric}</p>
                <p className={'pointer-events-none'}>{tLyric}</p>
            </div>
        )
    })
        ...
        return (
            <PageTransition>
                {player?.state == 'playing' && (
                    <div
                        className='artist-info padding-bottom-20 text-20 mb-8 mt-8 text-center font-medium text-neutral-400'>
